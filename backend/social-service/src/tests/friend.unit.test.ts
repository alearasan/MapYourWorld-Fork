import { sendRequestFriend, listFriends, findFriendById, listSearchUser, updateFriendStatus, getFriends, getPendingRequestsForRecipient } from '../services/friend.service';
...
describe('Friend Service', () => {
  ...
  describe('getFriends', () => {
    it("debe devolver una lista de amigos dado un ID de usuario", async () => {
      const friend: User = {
        id: 'user1',
      ...
      friendRepo.getFriends.mockResolvedValue([friend]);

      const result = await getFriends('user1');
      expect(friendRepo.getFriends).toHaveBeenCalledWith('user1');
      expect(result).toEqual([friend]);
    })
  });
  ...
  describe('getPendingRequestsForRecipient', () => {
    it("debe devolver una lista de peticiones de un usuario dado por su ID", async () => {
      ...
      friendRepo.getPendingRequestsForRecipient.mockResolvedValue([friendRequest]);

      const result = await getPendingRequestsForRecipient('friend1');
      expect(friendRepo.getPendingRequestsForRecipient).toHaveBeenCalledWith('friend1');
